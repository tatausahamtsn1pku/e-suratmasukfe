...
const getUserInfo = () => {
  try {
    const token = localStorage.getItem("authToken");
    if (!token) return { jabatan: "Kepala Tata Usaha", username: "", initial: "K" };
    const payload = JSON.parse(atob(token.split(".")[1]));
    return {
      jabatan:  payload.jabatan  || payload.position || "Kepala Tata Usaha",
      username: payload.username || payload.name     || "",
      initial:  (payload.username || payload.name || "K")[0].toUpperCase(),
    };
  } catch {
    return { jabatan: "Kepala Tata Usaha", username: "", initial: "K" };
  }
};
...
const api = {
  ...
  forwardToKamad:     (id, fd)   => fetch(`${API_BASE}/api/surat/forward-to-kamad/${id}`,       { method: "PATCH",  headers: getHeadersNoContentType(), body: fd }).then(r => r.json()),
  getAgendaMasuk:     ()         => fetch(`${API_BASE}/api/agenda/masuk`,                       { headers: getHeaders() }).then(r => r.json()),
  approveAgenda:      (id)       => fetch(`${API_BASE}/api/surat/agenda/approve/${id}`,         { method: "PATCH",  headers: getHeaders() }).then(r => r.json()),
};
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} maxW="max-w-sm">
      ...
      <div className="flex gap-3">
        <button onClick={onClose} className="flex-1 border-2 border-gray-200 text-gray-600 font-semibold py-3 rounded-2xl hover:bg-gray-50">Batal</button>
        <button onClick={onConfirm} disabled={loading}
          className="flex-1 bg-gradient-to-r from-[#6D9E51] to-[#4a7a36] text-white font-bold py-3 rounded-2xl disabled:opacity-60 flex items-center justify-center gap-2">
...
function MiniCalendar() {
  ...
  useEffect(() => {
    ...
    fetchCalDay(`${year}-${String(month + 1).padStart(2, "0")}-01`);
...
function DashboardView() {
  ...
  // Chart 3 — tren surat keluar (balasan)
  ...
  const statCards = [
    ...
    { label: "Menunggu Review KATU", value: getCount("AWAITING_KATU_REVIEW"),   sub: "Perlu ditindaklanjuti", icon: <Ic.Stamp />,      accent: "#2563eb" },
    { label: "Diteruskan ke Kamad",  value: getCount("AWAITING_KAMAD_APPROVAL"),sub: "Sudah diberi TTD",      icon: <Ic.Forward />,    accent: "#9333ea" },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Row 2: Tren Surat Masuk + Tren Surat Keluar */}
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={masukChartData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gMasukKATU" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%"  stopColor="#378ADD" stopOpacity={0.18} />
                      <stop offset="95%" stopColor="#378ADD" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="gSelesaiKATU" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="total" name="Surat Masuk"
                    stroke="#378ADD" strokeWidth={2} fill="url(#gMasukKATU)"
                  ...
                  <Area type="monotone" dataKey="completed" name="Selesai"
                    stroke="#1D9E75" strokeWidth={2} fill="url(#gSelesaiKATU)"
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={keluarChartData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gKeluarKATU" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="total" name="Surat Keluar"
                    stroke="#BA7517" strokeWidth={2} fill="url(#gKeluarKATU)"
      ...
      {!loading && (statsAgenda.masuk !== undefined || statsAgenda.keluar !== undefined) && (
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-center gap-6 flex-wrap"
          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-amber-50 rounded-xl flex items-center justify-center text-amber-600"><Ic.Mail /></div>
            <div>
...
function PDFViewer({ url, fileName }) {
  // States: "loading" | "blob" | "google" | "error"
  ...
  useEffect(() => {
    ...
    fetch(url, { headers })
      ...
      .then(blob => {
        // Ensure it's treated as PDF regardless of server Content-Type
      ...
      .catch(() => {
        // Fallback: Google Docs Viewer (no auth, works for public URLs)
        setViewState("google");
      });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-100 flex-shrink-0 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {viewState === "google" && (
            <button
              onClick={() => { if (blobUrl) setViewState("blob"); }}
              className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:border-[#6D9E51] hover:text-[#6D9E51] transition-all font-medium">
              Viewer Asli
            </button>
          )}
      ...
      <div className="flex-1 min-h-0 relative bg-gray-100" style={{ minHeight: "500px" }}>
        ...
        {viewState === "loading" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-gray-50">
            <div className="w-8 h-8 border-3 border-[#6D9E51] border-t-transparent rounded-full animate-spin" style={{ borderWidth: 3 }} />
            <p className="text-xs text-gray-400 font-medium">Memuat dokumen...</p>
          </div>
        )}

        {/* Blob URL iframe — renders without CORS issues */}
...
function SuratActionPanel({ suratId, onClose, onActionDone, toast }) {
  ...
  const isReviewable = detail?.status === "AWAITING_KATU_REVIEW";

  const handleForward = async () => {
    setActionLoading(true);
    try {
      const fd = new FormData();
      fd.append("catatanKATU", catatanKATU);
      if (ttdFile) fd.append("file", ttdFile);
      const res = await api.forwardToKamad(suratId, fd);
      if (res.success === false) throw new Error(res.message || "Gagal meneruskan ke Kamad");
      toast.add("Surat berhasil diteruskan ke Kepala Madrasah!");
  ...
  return (
    <div className="fixed inset-0 z-50" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(6px)", animation: "fadeIn 0.2s ease-out" }}>
      <div className="absolute inset-3 sm:inset-5 bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-white flex-shrink-0 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 bg-[#6D9E51]/10 rounded-xl flex items-center justify-center text-[#6D9E51] flex-shrink-0"><Ic.FileText /></div>
              ...
              <div className="border-t border-gray-100 p-4 space-y-2 bg-gray-50/40 flex-shrink-0">
                ...
                {isReviewable ? (
                  <button onClick={() => setShowForward(true)}
                    ...
                    <Ic.Stamp /> Tanda Tangani & Teruskan ke Kamad
                  </button>
                ) : (
      ...
      {/* Forward to Kamad modal */}
      <Modal open={showForward} onClose={() => setShowForward(false)} title="Tanda Tangani & Teruskan ke Kamad" maxW="max-w-md">
        <div className="space-y-4">
          <div className="bg-purple-50 border border-purple-100 rounded-xl p-3.5 flex items-start gap-2.5">
            <span className="text-purple-500 flex-shrink-0 mt-0.5"><Ic.Stamp /></span>
            <p className="text-xs text-purple-700 leading-relaxed">Upload file TTD (opsional) dan tambahkan catatan sebelum meneruskan ke Kepala Madrasah.</p>
          </div>

          {/* Upload TTD */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">File Tanda Tangan (PDF/Gambar)</label>
            <div onClick={() => ttdRef.current?.click()}
              className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all ${ttdFile ? "border-[#6D9E51] bg-[#FEFFD3]/40" : "border-gray-200 hover:border-[#BCD9A2] hover:bg-gray-50"}`}>
              ...
              {ttdFile ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">PDF, JPG, PNG — opsional</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Catatan KATU */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">Catatan KATU</label>
            <textarea value={catatanKATU} onChange={e => setCatatanKATU(e.target.value)}
              rows={3} placeholder="Tambahkan catatan atau instruksi untuk Kepala Madrasah (opsional)..."
...
function SuratView({ toast }) {
  ...
  const reviewCount = surat.filter(s => s.status === "AWAITING_KATU_REVIEW").length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4" style={{ animation: "fadeInUp 0.4s ease-out both" }}>
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Review dan teruskan surat ke Kepala Madrasah</p>
        </div>
        {reviewCount > 0 && (
          <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2">
            <span className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" />
            <span className="text-sm font-bold text-blue-700">{reviewCount} surat menunggu review KATU</span>
          </div>
        )}
      </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((s, i) => (
                    <tr key={s._id || s.id || i}
                      className={`transition-colors cursor-pointer group ${s.status === "AWAITING_KATU_REVIEW" ? "hover:bg-blue-50/50" : "hover:bg-[#FEFFD3]/30"}`}
                      onClick={() => setOpenId(s._id || s.id)}
                      style={{ animation: `fadeInUp 0.3s ease-out ${i * 30}ms both` }}>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          {s.status === "AWAITING_KATU_REVIEW" && <span className="w-1.5 h-1.5 bg-blue-500 rounded-full flex-shrink-0 animate-pulse" />}
...
// ─── Agenda View (KATU: read + approve) ──────────────────────────────────────
function AgendaView({ toast }) {
  const [agenda, setAgenda]         = useState([]);
  const [loading, setLoading]       = useState(true);
  const [search, setSearch]         = useState("");
  const [filterApproved, setFilter] = useState("ALL"); // "ALL" | "approved" | "pending"
  const [approveItem, setApproveItem] = useState(null);
  const [actionLoading, setActionLoading] = useState(false);

  const load = useCallback(() => {
    setLoading(true);
    api.getAgendaMasuk().then(d => {
      const data = d?.data ?? d ?? [];
  ...
  const handleApprove = async () => {
    setActionLoading(true);
    try {
      const itemId = approveItem._id || approveItem.id;
      const res = await api.approveAgenda(itemId);
      if (res.success === false) throw new Error(res.message || "Gagal menyetujui agenda");
      toast.add("Agenda berhasil disetujui!");
      setApproveItem(null); load();
    } catch (e) { toast.add(e.message, "error"); }
    setActionLoading(false);
  };

  const pendingCount = agenda.filter(a => !a.isAgendaDisetujuiKATU).length;

  const filtered = agenda.filter(a => {
    const matchSearch = !search || [a.nomorAgenda, a.nomorSurat, a.trackingId, a.instansi, a.namaPengirim]
      .some(v => v?.toLowerCase().includes(search.toLowerCase()));
    const matchFilter = filterApproved === "ALL"
      ? true : filterApproved === "approved"
      ? a.isAgendaDisetujuiKATU : !a.isAgendaDisetujuiKATU;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4" style={{ animation: "fadeInUp 0.4s ease-out both" }}>
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Crosscheck dan setujui catatan nomor agenda</p>
        </div>
        {pendingCount > 0 && (
          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2">
            <span className="w-2 h-2 bg-yellow-500 rounded-full animate-pulse" />
            <span className="text-sm font-bold text-yellow-700">{pendingCount} agenda menunggu persetujuan</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ animation: "fadeInUp 0.5s ease-out 100ms both" }}>
        <div className="p-4 sm:p-5 border-b border-gray-50 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <select value={filterApproved} onChange={e => setFilter(e.target.value)}
            className="border-2 border-gray-100 rounded-xl py-2.5 px-4 text-sm outline-none focus:border-[#6D9E51] bg-white text-gray-700 flex-shrink-0">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead><tr className="bg-gray-50/50">
                  {["Nomor Agenda","Tracking ID","Nomor Surat","Pengirim / Instansi","Tgl Agenda","Status","Aksi"].map((h, i) => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((a, i) => (
                    <tr key={a._id || a.id || i}
                      className={`transition-colors group ${!a.isAgendaDisetujuiKATU ? "hover:bg-yellow-50/40 cursor-pointer" : "hover:bg-[#FEFFD3]/20"}`}
                      style={{ animation: `fadeInUp 0.3s ease-out ${i * 30}ms both` }}>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          {!a.isAgendaDisetujuiKATU && <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full animate-pulse" />}
                      ...
                      <td className="px-5 py-3.5">
                        {a.isAgendaDisetujuiKATU ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-50 border border-yellow-200 text-yellow-700">
                            <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full" /> Menunggu
                          </span>
                        )}
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        {!a.isAgendaDisetujuiKATU && (
                          <button onClick={() => setApproveItem(a)}
                            className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg bg-[#6D9E51]/10 border border-[#BCD9A2] text-[#4a7a36] hover:bg-[#6D9E51]/20 transition-all opacity-0 group-hover:opacity-100">
                            <Ic.ShieldCheck /> Setujui
                          </button>
                        )}
                      </td>
                    </tr>
            ...
            <div className="md:hidden divide-y divide-gray-50">
              {filtered.map((a, i) => (
                <div key={a._id || a.id || i} className="p-4 space-y-2.5"
                  style={{ animation: `fadeInUp 0.3s ease-out ${i * 30}ms both` }}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <p className="text-sm font-bold text-gray-800">{a.namaPengirim || "—"}</p>
                      <p className="text-xs text-gray-400">{a.instansi} · {a.trackingId}</p>
                    </div>
                    {!a.isAgendaDisetujuiKATU && (
                      <button onClick={() => setApproveItem(a)}
                        className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl bg-[#6D9E51] text-white flex-shrink-0">
                        <Ic.Check /> Setujui
                      </button>
                    )}
                  </div>
      ...
      <ConfirmDialog
        open={!!approveItem} onClose={() => setApproveItem(null)} onConfirm={handleApprove}
        loading={actionLoading} title="Setujui Catatan Agenda"
        msg={`Anda akan menyetujui nomor agenda "${approveItem?.nomorAgenda}". Setelah disetujui, catatan ini tidak dapat diubah atau dihapus oleh Validator.`}
      />
    </div>
  );
}
...
// ─── Root ─────────────────────────────────────────────────────────────────────
export default function KATUDashboard() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50/80">
        ...
        <style>{`@media(min-width:768px){.katu-topbar,.katu-main{margin-left:${collapsed ? "64px" : "240px"}}}`}</style>

        <div className="katu-topbar fixed top-0 right-0 left-0 z-20 transition-all duration-300">
          <Topbar active={active} onMenuClick={() => setMobileOpen(true)} />
        </div>

        <main className="katu-main pt-16 min-h-screen transition-all duration-300">
