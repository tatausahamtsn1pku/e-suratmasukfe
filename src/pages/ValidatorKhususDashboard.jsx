...
// ─── User Info helper ─────────────────────────────────────────────────────────
const getUserInfo = () => {
  // Try to parse JWT payload for jabatan/username
  try {
    const token = localStorage.getItem("authToken");
...
const api = {
  ...
  // Agenda endpoints
  getAgenda:          ()         => fetch(`${API_BASE}/api/surat/agenda`,                        { headers: getHeaders() }).then(r => r.json()),
  createAgenda:       (id, body) => fetch(`${API_BASE}/api/surat/agenda/${id}`,                  { method: "POST",   headers: getHeaders(), body: JSON.stringify(body) }).then(r => r.json()),
  updateAgenda:       (id, body) => fetch(`${API_BASE}/api/surat/agenda/${id}`,                  { method: "PUT",    headers: getHeaders(), body: JSON.stringify(body) }).then(r => r.json()),
  deleteAgenda:       (id)       => fetch(`${API_BASE}/api/surat/agenda/${id}`,                  { method: "DELETE", headers: getHeaders() }).then(r => r.json()),
};
...
const Ic = {
  ...
  ZoomIn:       () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>,
  ...
  Expand:       () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>,
...
// ─── Modal ────────────────────────────────────────────────────────────────────
function Modal({ open, onClose, title, children, maxW = "max-w-lg", noPad = false }) {
  useEffect(() => {
...
// ─── Inline PDF Viewer ─────────────────────────────────────────────────────────
// Renders PDF from URL inside an iframe/embed without requiring download
      ...
      <div className="flex-1 min-h-0 relative bg-gray-100" style={{ minHeight: "500px" }}>
        ...
        {viewState === "blob" && blobUrl && (
          <iframe ref={iframeRef} src={blobUrl} className="w-full h-full border-0"
            style={{ minHeight: "500px" }} title={fileName || "Dokumen PDF"} />
        )}
        {viewState === "google" && googleUrl && (
          <iframe src={googleUrl} className="w-full h-full border-0"
            style={{ minHeight: "500px" }} title={fileName || "Dokumen PDF"}
    ...
    <div className="lg:hidden border-t border-gray-100 bg-white flex-shrink-0">
      ...
      {open && (
        <div className="border-t border-gray-100" style={{ height: "60vh" }}>
          ...
          {viewState === "blob" && blobUrl && (
            <iframe src={blobUrl} className="w-full h-full border-0" title={fileName || "PDF"} />
          )}
          {viewState === "google" && googleUrl && (
            <iframe src={googleUrl} className="w-full h-full border-0" title={fileName || "PDF"}
              sandbox="allow-scripts allow-same-origin allow-popups" />
          )}
          {(viewState === "error" || viewState === "idle") && (
            <div className="h-full flex flex-col items-center justify-center gap-3 p-6 text-center bg-gray-50">
              ...
              <a href={url} target="_blank" rel="noreferrer"
                className="flex items-center gap-2 bg-[#6D9E51] text-white text-sm font-bold px-4 py-2.5 rounded-xl">
                Buka di Tab Baru
              </a>
            </div>
          )}
...
// Split-pane: LEFT = info + actions, RIGHT = inline PDF viewer (always visible)
function SuratActionPanel({ suratId, onClose, onActionDone, toast }) {
  ...
  const canExecuteReject       = detail?.status === "AWAITING_KAMAD_APPROVAL"; // Kamad has instructed rejection
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      // Refresh detail
      const fresh = await api.getSuratDetail(suratId);
      setDetail(fresh?.data ?? fresh);
      setShowEdit(false);
    } catch (e) { toast.add(e.message, "error"); }
    setActionLoading(false);
  };
          ...
          <div className="flex-1 flex overflow-hidden min-h-0">
            ...
            <div className="w-full lg:w-80 xl:w-96 flex-shrink-0 flex flex-col border-r border-gray-100 overflow-y-auto">
              ...
              <div className="p-4 space-y-2.5 flex-1">
                ...
                {detail.catatanKATU && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-2.5">
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed">{detail.catatanKATU}</p>
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-gray-100 p-4 space-y-2 bg-gray-50/40 flex-shrink-0">
                ...
                {/* PENDING_VALIDATION: forward to KATU only */}
                {isPending && (
                ...
                {/* AWAITING_KAMAD_APPROVAL: Kamad has instructed rejection → execute it */}
                {canExecuteReject && (
                ...
                {/* AWAITING_REPLY_KHUSUS: send final reply */}
                {isReplyable && (
                ...
                {!isPending && !canExecuteReject && !isReplyable && (
            ...
            {/* ── RIGHT PANE: PDF viewer (always visible, no tab switch needed) ── */}
            <div className="hidden lg:flex flex-1 flex-col overflow-hidden bg-gray-100">
              {detail.fileUrl ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3 text-gray-400">
                  <Ic.FileText />
                  <p className="text-sm font-medium">Tidak ada file terlampir</p>
                </div>
              )}
            </div>

            {/* Mobile: show PDF below info via scrollable area */}
            {detail.fileUrl && (
              <div className="lg:hidden absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white to-transparent pointer-events-none" />
            )}
          </div>
        )}

        {/* Mobile PDF viewer — inline collapsible */}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea value={rejectAlasan} onChange={e => setRejectAlasan(e.target.value)}
              rows={4} placeholder="Contoh: Mohon maaf, format surat tidak sesuai standar. Silakan perbaiki dan ajukan kembali..."
...
function SuratView({ toast }) {
  ...
  // Priority badge for PENDING_VALIDATION
      ...
      <div className="flex items-center justify-between flex-wrap gap-4" style={{ animation: "fadeInUp 0.4s ease-out both" }}>
        ...
        {priorityCount > 0 && (
          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2">
            ...
            <span className="text-sm font-bold text-yellow-700">{priorityCount} surat menunggu validasi</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ animation: "fadeInUp 0.5s ease-out 100ms both" }}>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-sm font-medium">Tidak ada surat ditemukan</p>
          </div>
        ) : (
          <>
            {/* Desktop table */}
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((s, i) => (
                    <tr key={s._id || s.id || i}
                      className={`transition-colors cursor-pointer group ${s.status === "PENDING_VALIDATION" ? "hover:bg-yellow-50/60" : "hover:bg-[#FEFFD3]/30"}`}
...
function AgendaView({ toast }) {
  ...
  const [editItem, setEditItem] = useState(null);       // { id, nomorAgenda }
  const [deleteItem, setDeleteItem] = useState(null);   // { id, nomorAgenda }
  const [createSuratId, setCreateSuratId] = useState("");
  const [nomorInput, setNomorInput] = useState("");
  const [actionLoading, setActionLoading] = useState(false);

  // For creating agenda from a surat — we also need the surat list
  const [suratList, setSuratList] = useState([]);
  const [suratLoading, setSuratLoading] = useState(false);

  const load = useCallback(() => {
    setLoading(true);
    api.getAgenda().then(d => {
      const data = d?.data ?? d ?? [];
  ...
  const handleDelete = async () => {
    setActionLoading(true);
    try {
      const res = await api.deleteAgenda(deleteItem.id);
      if (res.success === false) throw new Error(res.message || "Gagal menghapus agenda");
      toast.add("Catatan agenda berhasil dihapus!"); setDeleteItem(null); load();
    } catch (e) { toast.add(e.message, "error"); }
    setActionLoading(false);
  };

  const filtered = agenda.filter(a =>
    !search || [a.nomorAgenda, a.nomorSurat, a.trackingId, a.instansi, a.namaPengirim]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4" style={{ animation: "fadeInUp 0.4s ease-out both" }}>
        ...
        <button
          onClick={() => { setNomorInput(""); setCreateSuratId(""); loadSurat(); setShowCreate(true); }}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ animation: "fadeInUp 0.5s ease-out 100ms both" }}>
        <div className="p-4 sm:p-5 border-b border-gray-50 flex items-center gap-3">
          <div className="relative flex-1 min-w-0">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300"><Ic.Search /></div>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari nomor agenda, tracking ID, instansi..."
              className="w-full border-2 border-gray-100 rounded-xl py-2.5 pl-9 pr-4 text-sm outline-none focus:border-[#6D9E51] transition-colors" />
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16"><div className="text-4xl mb-3">📋</div><p className="text-gray-400 text-sm font-medium">Belum ada catatan agenda</p></div>
        ) : (
          <>
            {/* Desktop table */}
            <div className="hidden md:block overflow-x-auto">
                      ...
                      <td className="px-5 py-3.5">
                        {a.isAgendaDisetujuiKATU ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-50 border border-yellow-200 text-yellow-700">
                            <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full animate-pulse" /> Menunggu Persetujuan
                          </span>
                        )}
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          {a.isAgendaDisetujuiKATU ? (
                            <span className="inline-flex items-center gap-1 text-xs text-gray-300 px-2 py-1">
                              <Ic.Lock /> Terkunci
                            </span>
                          ) : (
                            <>
                              <button onClick={() => { setEditItem(a); setNomorInput(a.nomorAgenda || ""); }}
                                className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition-all" title="Edit Nomor Agenda">
                                <Ic.Edit />
                              </button>
                              <button onClick={() => setDeleteItem(a)}
                                className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-all" title="Batalkan Agenda">
                                <Ic.Trash />
                              </button>
                            </>
                          )}
              ...
              <select value={createSuratId} onChange={e => setCreateSuratId(e.target.value)}
                ...
                {suratList.map(s => (
                  <option key={s._id || s.id} value={s._id || s.id}>
                    {s.trackingId} — {s.namaPengirim} ({s.instansi})
                  </option>
                ))}
              </select>
            )}
      ...
      {/* Edit modal */}
      <Modal open={!!editItem} onClose={() => setEditItem(null)} title="Revisi Nomor Agenda" maxW="max-w-sm">
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5 text-xs text-amber-700">
            Nomor agenda hanya bisa direvisi selama belum disetujui KATU.
          </div>
          <div>
            ...
            <input value={nomorInput} onChange={e => setNomorInput(e.target.value)}
              placeholder="Contoh: AGD-045-REVISI/MTsN/2026"
              className="w-full border-2 border-gray-200 rounded-xl py-3 px-4 text-sm outline-none focus:border-[#6D9E51] transition-colors" />
          </div>
          <div className="flex gap-3 pt-1">
            <button onClick={() => setEditItem(null)} className="flex-1 border-2 border-gray-200 text-gray-600 font-semibold py-3 rounded-xl hover:bg-gray-50 transition-all">Batal</button>
            <button onClick={handleEdit} disabled={actionLoading}
      ...
      <ConfirmDialog open={!!deleteItem} onClose={() => setDeleteItem(null)} onConfirm={handleDelete}
        loading={actionLoading} title="Batalkan Agenda"
        msg={`Catatan agenda "${deleteItem?.nomorAgenda}" akan dihapus. Tindakan ini hanya bisa dilakukan sebelum disetujui KATU.`} />
    </div>
  );
}
...
function Topbar({ active, onMenuClick }) {
  const titles = { dashboard: "Dashboard", surat: "Antrian Surat", agenda: "Buku Agenda" };
  const user = getUserInfo();
  return (
