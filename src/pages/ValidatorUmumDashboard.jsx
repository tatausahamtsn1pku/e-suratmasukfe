...
const getUserInfo = () => {
  try {
    const token = localStorage.getItem("authToken");
    if (!token) return { jabatan: "Validator Umum", username: "", initial: "V" };
    const payload = JSON.parse(atob(token.split(".")[1]));
    return {
      jabatan:  payload.jabatan  || payload.position || "Validator Umum",
      username: payload.username || payload.name     || "",
      initial:  (payload.username || payload.name || "V")[0].toUpperCase(),
    };
  } catch {
    return { jabatan: "Validator Umum", username: "", initial: "V" };
  }
};
...
function SuratActionPanel({ suratId, onClose, onActionDone, toast }) {
  ...
  return (
    <div className="fixed inset-0 z-50" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(6px)", animation: "fadeIn 0.2s ease-out" }}>
      <div className="absolute inset-3 sm:inset-5 bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-white flex-shrink-0 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 bg-cyan-50 rounded-xl flex items-center justify-center text-cyan-500 flex-shrink-0">
              <Ic.Reply />
            </div>
            <div className="min-w-0">
              <p className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Detail Surat — Validator Umum</p>
      ...
      {/* ── Reply Modal ── */}
      <Modal open={showReply} onClose={() => setShowReply(false)} title="Kirim Balasan Final" maxW="max-w-md">
        <div className="space-y-4">
          <div className="bg-[#6D9E51]/10 border border-[#BCD9A2] rounded-xl p-3.5 flex items-start gap-2.5">
            ...
            <p className="text-xs text-[#4a7a36] leading-relaxed">
              File PDF balasan akan dikirim langsung ke email pengirim surat secara otomatis.
            </p>
          </div>
          ...
          <div>
            ...
            <textarea value={replyPesan} onChange={e => setReplyPesan(e.target.value)}
              rows={3} placeholder="Tulis pesan balasan untuk pengirim (opsional)..."
            ...
            <div onClick={() => fileRef.current?.click()}
              ...
              {replyFile ? (
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-500">Klik untuk pilih file PDF balasan</p>
                  <p className="text-xs text-gray-400 mt-0.5">Format PDF, maksimal 10MB</p>
                </div>
              )}
            </div>
          </div>
...
function SuratView({ toast }) {
  ...
  const replyCount = surat.filter(s => s.status === "AWAITING_REPLY_UMUM").length;
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((s, i) => (
                    <tr key={s._id || s.id || i}
                      className={`transition-colors cursor-pointer group ${s.status === "AWAITING_REPLY_UMUM" ? "hover:bg-cyan-50/50" : "hover:bg-[#FEFFD3]/30"}`}
                      onClick={() => setOpenId(s._id || s.id)}
                      style={{ animation: `fadeInUp 0.3s ease-out ${i * 30}ms both` }}>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          {s.status === "AWAITING_REPLY_UMUM" && <span className="w-1.5 h-1.5 bg-cyan-500 rounded-full flex-shrink-0 animate-pulse" />}
...
// ─── Root ─────────────────────────────────────────────────────────────────────
export default function ValidatorUmumDashboard() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50/80">
        ...
        <style>{`@media(min-width:768px){.vu-topbar,.vu-main{margin-left:${collapsed ? "64px" : "240px"}}}`}</style>

        <div className="vu-topbar fixed top-0 right-0 left-0 z-20 transition-all duration-300">
          <Topbar active={active} onMenuClick={() => setMobileOpen(true)} />
        </div>

        <main className="vu-main pt-16 min-h-screen transition-all duration-300">
